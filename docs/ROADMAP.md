...
## Status atual (V1 hardening + V1.1)

- Sessão STT serializada até o fim da transição de HUD/injeção (reduz race conditions de start/stop).
- Injeção com telemetria detalhada de latência por etapa (`resolve_window_ms`, `paste_attempt_ms`, `clipboard_restore_ms`).
- HUD atualizado para renderização imperativa em `canvas` (menos custo de render contínuo).
- Histórico local implementado:
  - persistência em `history.json` no `userData`
  - busca, cópia, remoção e limpeza no renderer
...
- Não armazenar áudio; histórico de texto local ligado por default (retenção 30 dias).
