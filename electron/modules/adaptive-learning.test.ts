...
describe('adaptive learning suggestions', () => {
  it('suggests format style, language bias and protected term for repeated app patterns', () => {
    const suggestions = generateAdaptiveSuggestions(
      {
        apps: {
          slack: {
            ...
            termStats: {
              workspace: { term: 'Workspace', count: 5, lastSeenAt: '2026-03-07T00:00:00.000Z' },
            },
          },
        },
    ...
    expect(suggestions.some((item) => item.type === 'language-bias')).toBe(true);
    expect(suggestions.some((item) => item.type === 'protected-term')).toBe(true);
  });
});
