  ...
  try {
    ...
    if (response.status === 401 || response.status === 403) {
      return {
        status: "auth-error",
        message:
          "Azure Speech rejeitou a autenticação. Verifique chave e região configuradas.",
        host,
      };
    }
...
function checkHookHealth(args: {
  ...
  if (!args.holdToTalkEnabled) {
    return {
      ...
      status: "warn",
      message:
        "O modo segurar para falar está desativado pela configuração atual.",
    };
  }
  ...
  if (!args.holdHookActive) {
    return {
      ...
      status: "error",
      message:
        "O atalho global não carregou. Use a recuperação automática para restaurar o PTT.",
    };
  }
...
export async function getHealthCheckReport(args: {
        ...
        : connectionResult?.status === "network-error"
          ...
          : {
              ...
              message:
                ...
                "Azure Speech não pôde ser validado.",
            };
  ...
  const networkItem: HealthCheckItem = sttError
    ? {
        ...
        status: "warn",
        message:
          "Configuração do Azure ausente. O teste de rede do STT foi ignorado.",
      }
    : !args.includeExternalAzureCheck
      ? {
          id: "network",
  ...
  const historyItem: HealthCheckItem = {
    id: "history",
    status:
      args.privacyMode || args.historyStorageMode === "plain" ? "warn" : "ok",
  ...
  const injectionItem: HealthCheckItem = args.recentInjection
    ...
    : {
        ...
        status: "warn",
        message:
          "Ainda não há dados recentes sobre inserção automática de texto.",
      };
