...
function isAllowedNavigation(url: string, devServerUrl?: string) {
  ...
  return getTrustedAppOrigins(devServerUrl)
    ...
    .some(
      (origin) => url === origin || url.startsWith(withTrailingSlash(origin)),
    );
}
...
function extractRequestOrigin(
  ...
  const requestingOrigin =
    typeof record.requestingOrigin === "string"
      ? record.requestingOrigin
      : undefined;
  const securityOrigin =
    typeof record.securityOrigin === "string"
...
export function installSessionSecurity(
  ...
  session.setPermissionCheckHandler(
    (webContents, permission, requestingOrigin, details) => {
      const origin =
        requestingOrigin || extractRequestOrigin(webContents, details);
      return isAllowedPermissionRequest(
        permission,
        origin,
        details,
        devServerUrl,
