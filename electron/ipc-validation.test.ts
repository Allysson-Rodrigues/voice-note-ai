...
describe("ipc validation", () => {
  ...
  it("rejects unknown settings keys", () => {
    expect(() => validateSettingsUpdate({ invalid: true })).toThrow(
      /nao permitido/i,
    );
  });
  ...
  it("accepts valid dictionary payload", () => {
    expect(
      validateDictionaryAddPayload({ term: "Workspace", hintPt: "produto" }),
    ).toEqual({
      term: "Workspace",
      hintPt: "produto",
    });
  ...
  it("accepts valid stt start payload", () => {
    expect(
      validateSttStartPayload({ sessionId: "abc-123", language: "pt-BR" }),
    ).toEqual({
      sessionId: "abc-123",
      language: "pt-BR",
    });
  ...
  it("accepts valid Azure credential payload", () => {
    expect(
      validateAzureCredentialsPayload({ key: "abc", region: "brazilsouth" }),
    ).toEqual({
      key: "abc",
      region: "brazilsouth",
    });
