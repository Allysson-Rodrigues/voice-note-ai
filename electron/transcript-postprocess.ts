...
function applyExplicitFormattingCommands(value: string) {
  ...
  next = next.replace(
    /\b(?:item|n[uú]mero|numero|number)\s+(\d{1,2})\b/gi,
    (_entry, n: string) => `\n${n}.`,
  );
  ...
  next = next.replace(
    /\b(?:interrogação|interrogacao|ponto de interrogação|ponto de interrogacao)\b/gi,
    '?',
  );
  next = next.replace(
    /\b(?:exclamação|exclamacao|ponto de exclamação|ponto de exclamacao)\b/gi,
    '!',
  );
...
export function applyTranscriptPostprocess(rawText: string, options: TranscriptPostprocessOptions) {
  ...
  const withCommands =
    options.formatCommandsEnabled === false
      ? withCanonical
      : applyExplicitFormattingCommands(withCanonical);
