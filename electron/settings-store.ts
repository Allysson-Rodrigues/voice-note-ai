...
function parseCanonicalTerms(raw: unknown): CanonicalTerm[] {
  ...
  for (const item of raw) {
    ...
    const from = normalizePhrase(typeof (item as { from?: unknown }).from === "string" ? (item as { from: string }).from : "");
    const to = normalizePhrase(typeof (item as { to?: unknown }).to === "string" ? (item as { to: string }).to : "");
    if (!from || !to) continue;
...
function parseSettings(raw: unknown): AppSettings {
  ...
  const toneMode: ToneMode =
    obj.toneMode === "formal" ? "formal" : obj.toneMode === "very-casual" ? "very-casual" : "casual";
