...
describe('hotkey helpers', () => {
  it('maps accelerator to user label', () => {
    expect(hotkeyLabelFromAccelerator('CommandOrControl+Super')).toBe('Ctrl+Win');
    expect(hotkeyLabelFromAccelerator('CommandOrControl+Super+Space')).toBe('Ctrl+Win+Space');
  });
